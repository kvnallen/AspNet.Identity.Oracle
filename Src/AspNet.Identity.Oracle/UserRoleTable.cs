...
namespace AspNet.Identity.Oracle
{
    ...
    public class UserRolesTable
    {
        ...
        {
            const string commandText = @"SELECT NAME FROM IDSSO_USERROLES, IDSSO_ROLES WHERE USERID = :USERID AND IDSSO_USERROLES.ROLEID = IDSSO_ROLES.ID";
        ...
        {
            const string commandText = @"DELETE FROM IDSSO_USERROLES WHERE USERID = :USERID";
            var parameters = new List<OracleParameter>
            {
                new OracleParameter{ ParameterName = "USERID", Value = userId, OracleDbType = OracleDbType.Varchar2 }
            };
        ...
        {
            const string commandText = @"INSERT INTO IDSSO_USERROLES (USERID, ROLEID) VALUES (:USERID, :ROLEID)";
            var parameters = new List<OracleParameter>
            {
