...
namespace AspNet.Identity.Oracle
{
		...
		where TUser : IdentityUser
	{
		...
		{
			const string commandText = @"SELECT NAME FROM IDSSO_USERS WHERE ID = :USERID";
		...
		{
			const string commandText = @"SELECT ID FROM IDSSO_USERS WHERE USERNAME = :NAME";
			var parameters = new List<OracleParameter>
			{
		...
		{
			TUser user;
			const string commandText = @"SELECT * FROM IDSSO_USERS WHERE ID = :USERID";
			var parameters = new List<OracleParameter>
			{
				new OracleParameter{ ParameterName = "USERID", Value = userId, OracleDbType = OracleDbType.Varchar2 }
			};
		...
		public List<TUser> GetUserByName(string userName)
		{
			...
			const string commandText = @"SELECT * FROM IDSSO_USERS WHERE USERNAME = :NAME";
			var parameters = new List<OracleParameter>
			{
				new OracleParameter{ ParameterName = "NAME", Value = userName, OracleDbType = OracleDbType.Varchar2}
			};
		...
		{
			const string commandText = @"SELECT PASSWORDHASH FROM IDSSO_USERS WHERE ID = :USERID";
			var parameters = new List<OracleParameter>
			{
				new OracleParameter{ ParameterName = "USERID", Value = userId, OracleDbType = OracleDbType.Varchar2 }
			};
		...
		{
			const string commandText = @"UPDATE IDSSO_USERS SET PASSWORDHASH = :PWHASH WHERE ID = :USERID";
			var parameters = new List<OracleParameter>
			{
		...
		{
			const string commandText = @"SELECT SECURITYSTAMP FROM IDSSO_USERS WHERE ID = :USERID";
			var parameters = new List<OracleParameter>
			{
				new OracleParameter{ ParameterName = "USERID", Value =  userId, OracleDbType = OracleDbType.Varchar2 }
			};
		...
		{
			const string commandText = @"INSERT INTO IDSSO_USERS (USERNAME, ID, PASSWORDHASH, SECURITYSTAMP,EMAIL,EMAILCONFIRMED,PHONENUMBER,PHONENUMBERCONFIRMED, ACCESSFAILEDCOUNT,LOCKOUTENABLED,LOCKOUTENDDATEUTC,TWOFACTORENABLED, ENABLED)
                VALUES (:NAME, :USERID, :PWHASH, :SECSTAMP,:EMAIL,:EMAILCONFIRMED,:PHONENUMBER,:PHONENUMBERCONFIRMED,:ACCESSFAILEDCOUNT,:LOCKOUTENABLED,:LOCKOUTENDDATEUTC,:TWOFACTORENABLED, :ENABLED)";
			var parameters = new List<OracleParameter>
			{
				...
				new OracleParameter{ ParameterName = "ENABLED", Value = user.Enabled.ToDecimal(), OracleDbType = OracleDbType.Decimal },
			};
		...
		{
			const string commandText = @"DELETE FROM IDSSO_USERS WHERE ID = :USERID";
			var parameters = new List<OracleParameter>
			{
				new OracleParameter {ParameterName = "USERID", Value = userId, OracleDbType = OracleDbType.Varchar2},
			};
		...
		public bool IsEnabled(string username)
		{
			...
			return _database.QueryValue(commandText, parameters) != null;
		}
		...
		{
			const string commandText = @"UPDATE IDSSO_USERS SET
                USERNAME =:NAME,
                PASSWORDHASH =:PWHASH,
		...
		{
			var users = new List<TUser>();
			const string commandText = @"SELECT * FROM IDSSO_USERS";
